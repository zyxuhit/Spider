...
def spider_get(url, params):
    ...
    try:
        ...
        request.add_header(
            "Cookie", "_ulta_id.CM-Prod.e9dc=a945893c01c22fe2;_ulta_ses.CM-Prod.e9dc=afd85e03ece9757f;lkNrr3VQZJ=MDAwM2IyNTRjZDAwMDAwMDAwNDUwUEpoMBAxNTQwNDQxOTgx")
        request.add_header('Host', "www.chinamoney.com.cn")
        request.add_header('Origin', "http://www.chinamoney.com.cn")
        request.add_header(
            'Referer', "http://www.chinamoney.com.cn/chinese/jgxxpl/?entyDefinedCode=300868")
        request.add_header(
...
def deal_spider(item_tag, item_name, item_type, data):
    ...
    if((item_type == "重大事项") or (item_type == "财务报告") or (item_type == "评级报告")):
        ref = [[item_tag, item_name, item_type, d["releaseDate"].split()[0], d["title"], "http://www.chinamoney.com.cn/dqs/cm-s-notice-query/fileDownLoad.do?contentId=" +
                                                                       d["contentId"]+"&priority=0&mode=open"] for d in data["data"]["resultList"]]
        df = pd.DataFrame(ref, columns=[
                          'entyDefinedCode', 'entyFullName', "公告类型", "发布日期", "公告标题", "下载地址"])
        return df
    else:
        """
        ...
        df = pd.DataFrame(columns=[
                          'entyDefinedCode', 'entyFullName', "公告类型", "发布日期", "债券全称", "公告标题", "下载地址"])
        for t in data["data"]["resultList"]:
            ...
            ref1 = [[item_tag, item_name, item_type, d["releaseDate"].split()[0], ztitle, d["title"], "http://www.chinamoney.com.cn/dqs/cm-s-notice-query/fileDownLoad.do?contentId=" +
                                                                            d["contentId"]+"&priority=0&mode=open"] for d in others["data"]["resultList"] if ((d["suffix"] != None) and (d["attSize"] == 1))]

            ref2 = [[item_tag, item_name, item_type, d["releaseDate"].split()[0], ztitle, d["title"], "http://www.chinamoney.com.cn"+d["draftPath"]]
                                                                            for d in others["data"]["resultList"] if d["suffix"] == None]
            ref3 = []
            for d in others["data"]["resultList"]:
                if ((d["suffix"] != None) and (d["attSize"] > 1)):
                    for i in range(0, 10):
                        url4 = "http://www.chinamoney.com.cn/dqs/cm-s-notice-query/fileDownLoad.do?mode=save&contentId=" + \
                            d["contentId"]+"&priority="+str(i)
                        try:
                            ...
                            if (data != None):
                                ...
                                data = data.encode('latin1').decode(
                                    'utf8').split(".")[0]
                                ref3.append([item_tag, item_name, item_type, d["releaseDate"].split()[
                                            0], ztitle, data, url4])
                        except (urllib.error.HTTPError, urllib.error.URLError) as e:
                            print(e)
            ref2.extend(ref1)
...
# 获取实体名称
    ...
    fxtype = [["zqpj", "中期票据", 100010], ["dqrzq", "短期融债券", 100006], ["qyz", "企业债", 100004], ["tycd", "同业存单", 100041], ["zczczq", "资产支持证券", 999999], ["fgkdxzwrzgj", "非公开定向债务融资工具", 100050], [
        "ejzbgj", "二级资本工具", 100054], ["cdqrzq", "超短期融资券", 100029], ["ptjrz", "普通金融债", 100007], ["dfzfz", "地方政府债", 100011], ["decd", "大额存单", 100058], ["xmsyzq", "项目收益债券", 100057], ]

    item_name = None
    fx = pd.DataFrame(columns=[
                      'entyDefinedCode', 'entyFullName', "公告类型", "发布日期", "债券全称", "公告标题", "下载地址"])
    for t in fxtype:
        params = {
        "secondType": "0601,0602,0701,1001,1103,1104,1301,1302,1303,1304,1801,1802,1901,1902,2001,2002",
        ...
        if(fxxx["data"]["total"] != 0):
            if (fxxx["data"]["resultList"][0]["bondFullName"].split("2", maxsplit=1)[0] != None):
                item_name = fxxx["data"]["resultList"][0]["bondFullName"].split("2", maxsplit=1)[
                                                                                0]
                print(item_name)
                break
        else:
    ...
    if (item_name == None):
        ...
        if(zdsx["data"]["total"] != 0):
            ...
            if (zdsx["data"]["resultList"][0]["title"].split("2", maxsplit=1)[0] != None):
                item_name = zdsx["data"]["resultList"][0]["title"].split("2", maxsplit=1)[
                                                                         0]
                print(item_name)
